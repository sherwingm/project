...
interface JoinSharedGroupProps {
  token: string;
  onJoinSuccess: (group: Group) => void;
  onLoginRequested: (invitePath: string) => void;
}

export default function JoinSharedGroup({ token, onJoinSuccess, onLoginRequested }: JoinSharedGroupProps) {
  const { user } = useAuth();
  const [preview, setPreview] = useState<SharedGroupResponse | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [isJoining, setIsJoining] = useState(false);
  const [error, setError] = useState('');

  const invitePath = useMemo(() => `/share/${token}/join`, [token]);
  ...
  useEffect(() => {
    ...
    const loadPreview = async () => {
      try {
        setIsLoading(true);
        setError('');
        const data = await apiService.getSharedGroup(token);
        if (!cancelled) {
          setPreview(data);
        }
      } catch (err) {
  ...
  const handlePrimaryAction = async () => {
    ...
    try {
      setIsJoining(true);
      setError('');
      const joinedGroup = await apiService.joinGroupByToken(token);
        ...
        <div className="dark-card w-full rounded-[32px] p-6 shadow-[0_30px_90px_rgba(0,0,0,0.42)] sm:p-8">
          ...
          <div className="mt-5 space-y-3">
            ...
            <p className="max-w-xl text-sm text-slate-300 sm:text-base">
              Review this invite and confirm to become a real member of the group.
            </p>
          </div>

          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-4">
              <p className="text-xs uppercase tracking-[0.24em] text-slate-300">Group</p>
              <p className="mt-1 text-lg font-semibold text-white">{preview?.groupName || '—'}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-4">
              <p className="text-xs uppercase tracking-[0.24em] text-slate-300">Members</p>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            <button
              type="button"
              onClick={() => window.location.assign(`/share/${token}`)}
          ...
          <div className="mt-6 rounded-2xl border border-amber-400/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
            ...
            If you are not logged in, we will save this invite and return here after login.
          </div>
        </div>
      </div>
    </div>
